...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		Intent loadExplicitly = new Intent(ActivityLoaderActivity.this, ExplicitlyLoadedActivity.class);
		
		// TODO - Start an Activity using that intent and the request code defined above
		startActivityForResult(loadExplicitly, GET_TEXT_REQUEST_CODE);
	...
	private void startImplicitActivation() {
		...
		Intent baseIntent = new Intent(Intent.ACTION_VIEW, link);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == GET_TEXT_REQUEST_CODE) {
			//check that the result is actually ok
			if (resultCode == RESULT_OK) {
				
				//change the textview's text with the getDataResults
				mUserTextView.setText(data.getStringExtra("Input Text"));
			}
		}
